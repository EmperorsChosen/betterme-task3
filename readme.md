...
Потім необхідно запустити скрипт courses.py - він затягне в базу і створить таблицю з курсами валют до долара. Це потрібно для подальшого підрахунку суми в одному із завдань по запитам

Всі подальші трансофрмації я вирішив робити безспосередньо в БД. 
Пайтонівський скрипт просто перекидує з ТАБ-сепарейтед вальюс архіву в одну табличку яка повністю повторює структуру.

Казати що це єдиний правильний спосіб не буду - я робив так бо мені звичніше проводити маніпуляції з даними на стороні бази.
Теоретично при більших об'ємах (не 120 КБ) як в прикладі такий підхід кращий, бо при перетворенні даних через пандас ми витратимо весь запас оперативної пам'яті який ще не купив OpenAI
